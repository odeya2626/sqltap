...
class QueryStats(object):
    ...
    def __init__(
        self, text, stack, start_time, end_time, user_context, params_dict, results
    ):
        self.text = text
...
class ProfilingSession(object):
    ...
    def __init__(
        self, engine=sqlalchemy.engine.Engine, user_context_fn=None, collect_fn=None
    ):
    ...
    def _after_exec(
        self, conn, clause, multiparams, params, execution_options, results
    ):
        """SQLAlchemy event hook"""
        ...
        context = (
            None
            if not self.user_context_fn
            else self.user_context_fn(conn, clause, multiparams, params, results)
        )
    ...
    def collect(self):
        ...
        if not self.collector:
            raise AssertionError(
                "Can't call collect when you've registered " "your own collect_fn!"
            )
...
class QueryGroup(object):
    ...
    def add_params(self, q):
        ...
        count, params_id, params = self.params_hashes.get(key, (0, None, q.params))
...
class Reporter(object):
    ...
    def _init_template(self, template_filters=["unicode", "h"]):
        ...
        lookup = mako.lookup.TemplateLookup(
            self.template_dir, default_filters=template_filters
        )
    ...
    def _process_stats(self):
        ...
        for qstats in self.stats:
            qstats.stack_text = "".join(traceback.format_list(qstats.stack)).strip()
...
def report(statistics, filename=None, template="html.mako", **kwargs):
    ...
    if report_format:
        ...
        if report_format not in REPORTER_MAPPING.keys():
            raise Exception(
                "Format |%s| is not valid! formats supported: %s ",
